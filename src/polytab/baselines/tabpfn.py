...
class SimpleTabPFNYPredictor:
    def __init__(self, device='cuda', random_state=42, test_size=0.2):
        ...
        self.models = {}  # Store TabPFNRegressor models, one per column
        self.Y_columns = []
        self.low_variance_cols = []

        # Store statistics strictly from the training set
        ...
        print(f"🔧 Core architecture: TabPFNRegressor (Baseline)")
        ...
        print(f"⚙️ Runtime device: {self.device}")
    ...
    def _assign_class_labels(self, Y_data, missing_mask_new):
        """Keep interval-assignment logic; although no classifier is used, it is used to generate class_probabilities files for comparison"""
    ...
    def load_and_process_data(self, csv_file_path, smiles_column='psmiles',
        ...
        BERT-related feature extraction has been removed.
        """
        ...
        # 3. Compute quantiles using only retained training-set positions
        ...
        # 4. Create output directory with a _TabPFN suffix for distinction
        ...
        # Save GT files using the same format as the original code
    ...
    def train_model(self, y_data, missing_mask_new):
        """
        Core training logic (TabPFN version, revised):
        ...
        3. Train one TabPFNRegressor for each Y column
        """
        print(f"\n🚀 Starting TabPFN regression model training (device: {self.device}）...")
        ...
        # --- 3. Train TabPFNRegressor models in a loop ---
        ...
        for j, col_name in enumerate(self.Y_columns):
            ...
            # Fit model
            # Note: TabPFN usually does not require extensive hyperparameter tuning because it uses Transformer priors
            model.fit(X_scaled[self.train_indices], target_j)

            self.models[col_name] = model

            # Predict the full dataset
            # TabPFN supports batch prediction, so the full X_scaled can be passed directly
            final_predictions_scaled[:, j] = model.predict(X_scaled)

            if (j + 1) % 1 == 0 or (j + 1) == n_cols:
                print(f"  [Progress] completed {j+1}/{n_cols} column models trained (Target: {col_name})")

        # Inverse-transform predictions to the original scale
        self.full_predictions = self.scaler_Y_target.inverse_transform(final_predictions_scaled)
        print("✅ TabPFN regression training and full-set prediction complete")
    ...
    def visualize_train_results(self, train_predictions, y_data, missing_mask_new, save_dir="."):
        ...
        plt.show() # Usually commented out for batch runs

    def visualize_test_results(self, test_predictions, y_data, missing_mask_new, save_dir="."):
        """Fully aligned with the test-set visualization logic of 126.py"""
    ...
    def plot_training_curve(self, save_dir="."):
        """TabPFN is a Transformer forward-inference model with no iterative loss curve; generate an explanatory plot to keep the file structure consistent"""
    ...
    def run_pipeline(self, csv_file_path, smiles_column='psmiles', keep_mode='keep1', task_key=None):
        ...
        print("=" * 60)
        print(f"🧪 TabPFN Baseline prediction system | strategy: {keep_mode}")
        print("=" * 60)
        ...
        # ====== Added: compute and print mean R² ======
        if not train_res.empty:
            mean_train_r2 = train_res['R²'].mean()
            print(f"\n📈 [Summary] mean R² across all training-set features: {mean_train_r2:.4f}")
        
        if not test_res.empty:
            mean_test_r2 = test_res['R²'].mean()
            print(f"📈 [Summary] mean R² across all test-set features: {mean_test_r2:.4f}")
        # ====================================
        ...
        if not train_res.empty and not test_res.empty:
            ...
            # ====== Added: include a mean row in the CSV comparison table ======
            ...
            print("\n📊 Performance comparison (including each column and the mean):")
...
# --- Example run, aligned with the 126.py logic ---
if __name__ == "__main__":
    # Initialize the TabPFN predictor and specify device
    ...
    for km in ['keep1', 'keep2','keep3', 'keep4', 'keep25', 'keep50', 'keep75']:
        if os.path.exists(csv_path):
            ...
            print(f"⚠️ File not found: {csv_path}, please make sure the file is in the current directory.")
            break
