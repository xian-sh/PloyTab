...
# Remove torch-related imports and use a pure sklearn architecture
...
class SimpleBayesianYPredictor:
    def __init__(self, device=None, random_state=42, test_size=0.2):
        ...
        1. Remove PolyBERT feature extraction
        2. Remove the main-model attention and cascade-regression modules
        3. Use only tabular Y values and mask status as features
        """
        ...
        self.models = {}  # Store BayesianRidge models, one per column
        self.Y_columns = []
        ...
        print(f"🔧 Core architecture: Scikit-learn BayesianRidge (Baseline)")
    ...
    def _build_keep_mask(self, n_rows, n_cols, keep_k, rng):
        """Keep the mask-generation logic from 126.py unchanged"""
    ...
    def _compute_quantile_boundaries(self, Y_data, missing_mask_new):
        """Keep the quartile-computation logic from 126.py for subsequent classification-file saving"""
        quantile_boundaries = {}
    ...
    def load_and_process_data(self, csv_file_path, smiles_column='psmiles',
        ...
        # 4. Create output directory with a _Bayesian suffix to distinguish this experiment
    ...
    def train_model(self, y_data, missing_mask_new):
        ...
        2. Manually set the StandardScaler var_ attribute to avoid NotFittedError
        """
        print("\n🚀 Starting training with median imputation...")

        n_rows, n_cols = y_data.shape
        
        # 1. Initialize the imputation matrix
        y_data_filled = y_data.copy()
        self._train_medians = [] 

        # 2. Compute training-set medians column by column and fill missing values
        ...
        # --- 3. Compute statistics and manually initialize the scaler ---
        ...
        var_arr = scales_arr ** 2  # <--- Key fix: compute variance
        ...
        self.scaler_Y_input.n_samples_seen_ = np.array([len(self.train_indices)] * n_cols) # optional, more robust
        ...
        # 5. Train BayesianRidge models in a loop
        ...
        for j, col_name in enumerate(self.Y_columns):
            ...
            model = BayesianRidge()
            # Note: X contains all features after imputation
        ...
        print("✅ Bayesian regression training and full-set prediction complete")
    ...
    def plot_training_curve(self, save_dir="."):
        """The Bayesian model has no training curve; generate a placeholder plot to keep the file structure consistent"""
    ...
    def run_pipeline(self, csv_file_path, smiles_column='psmiles', keep_mode='keep1', task_key=None):
        ...
        print("=" * 60)
        print(f"🧪 Bayesian Baseline prediction system | strategy: {keep_mode}")
        print("=" * 60)
        ...
        # Save CSV results using exactly the same naming as 126.py
