# -*- coding: utf-8 -*-
# Auto-generated from NEWMD3-run_polyBERT_cascade_260107.ipynb
...
class SimpleSMILESYPredictor:
    ...
    def load_and_process_data(self, csv_file_path, smiles_column='psmiles',
        ...
        save_dir = os.path.join("ckpts", f"{task_key}_polyBERT_cascade_{keep_mode}_260107_SAFE")
    ...
    def train_model(self, features, df_complete, df_missing, missing_mask_new, class_labels,
                    epochs=500, batch_size=1024, lr=0.002):
        """训练模型（仅用训练集，无数据泄漏）"""
    ...
    def run_pipeline(self, csv_file_path, smiles_column='psmiles',
        ...
        try:
            ...
            losses = self.train_model(
                ...
                missing_mask_new, class_labels, epochs=500, batch_size=1024
            )
...
predictor = SimpleSMILESYPredictor(
    random_state=42,
    alpha_non_missing_loss=0.2,
    lambda_vae=0.05,
    beta_vae=0.15,
    vae_latent_dim=128,
    bert_model_path=POLYBERT_MODEL_PATH,
    lambda_class=1.0,
    lambda_cascade=[0.8, 0.6, 0.4, 1.0],
    test_size=0.2  # 20%测试集
)

# 2. 配置数据路径
data_dict = {
    'newmd': 'polymer_MD.csv'
}
smiles_col_dict = {
    'newmd': 'smiles_list'
}
